...
/// Serialize the policies as Rspamd settings-module content for
/// `local.d/settings.conf`.
///
/// Rspamd wraps `local.d/settings.conf` in the implicit `settings { }` section
/// itself, so we emit the per-tenant blocks WITHOUT an outer wrapper — a
/// wrapper here produces illegal `settings { settings { … } }` nesting (caught
/// by `rspamadm configtest`).
fn render_ucl(policies: &[TenantPolicy]) -> String {
    let mut s = String::new();
    s.push_str("# GENERATED by opengroupware config-compiler — DO NOT EDIT.\n");
    s.push_str("# Rspamd settings-module content for local.d/settings.conf.\n");
    s.push_str("# Source: control-plane desired state (active tenants + domains).\n");
    for p in policies {
        // Rspamd `rcpt` matches the full recipient address. A bare domain does
        // NOT match `user@domain`; we emit a domain-anchored regex per domain
        // (`/@domain$/`, dots escaped) so the tenant's settings apply to every
        // address at that domain. Verified against a live rspamd (settings_id
        // + required_score reflect the matched tenant).
        let rcpt = p
            .domains
            .iter()
...
mod tests {
    ...
    fn ucl_contains_domains_and_settings_ids() {
        ...
        // domain-anchored regex rcpt, dots escaped
